...
SPEC_GUIDANCE_MODE = os.getenv("SPEC_GUIDANCE_MODE", "independent").strip().lower()
if SPEC_GUIDANCE_MODE not in {"signature_only", "independent", "executable_reference"}:
    SPEC_GUIDANCE_MODE = "independent"
ENABLE_PROOF_REPAIR = os.getenv("ENABLE_PROOF_REPAIR", "1") != "0"
MAX_PROOF_REPAIR_ATTEMPTS = int(os.getenv("MAX_PROOF_REPAIR_ATTEMPTS", "1"))
MAX_VERIFICATION_SPEC_REPAIR_ROUNDS = int(
    os.getenv("MAX_VERIFICATION_SPEC_REPAIR_ROUNDS", "1")
)
...
ENABLE_MUTATION_SPEC_STRENGTHENING = os.getenv("ENABLE_MUTATION_SPEC_STRENGTHENING", "0") != "0"
...
CRITIC_GATE_MODE = os.getenv("CRITIC_GATE_MODE", "advisory").strip().lower()
if CRITIC_GATE_MODE not in {"advisory", "strict"}:
    CRITIC_GATE_MODE = "advisory"
...
TARGET_END_TO_END_RATE = float(os.getenv("TARGET_END_TO_END_RATE", "0.60"))
